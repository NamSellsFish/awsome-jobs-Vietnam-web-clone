...
ses = requests.get(f"https://github.com/awesome-jobs/vietnam/issues?page=2&q=is%3Aissue+is%3Aopen")
...
details = [f"https://github.com{detail.attrs.get('href')}" for detail in
           soup.find_all("a", class_="Link--primary v-align-middle no-underline h4 js-navigation-open markdown-title")]
openedDates = [date.text for date in soup.find_all("relative-time", class_="no-wrap")]
jobs = []
...
if res.fetchone() is None:
    cur.execute("CREATE TABLE jobs(titles, opened, details)")
    for job in jobs:
        cur.execute(f"""
...
def showTheJobs():
    ...
    for top, row in enumerate(cur.execute("SELECT titles,opened,details FROM jobs ORDER BY titles"), 1):
        jobsJSON.append({"No": top, "title": row[0], "opened": row[1], "detail": row[2]})
